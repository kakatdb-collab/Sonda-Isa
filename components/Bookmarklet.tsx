...
export const Bookmarklet: React.FC = () => {
  ...
  const consoleScript = 
    "(function() { " +
    "  try { " +
    "    console.log('⚡ Sonda Isa v41 Iniciado'); " +
    "    /* Limpeza de versões anteriores */ " +
    "    var ids = ['li-extractor-v35', 'li-extractor-v36', 'li-extractor-v37', 'li-extractor-v38', 'li-extractor-v39', 'li-extractor-v40', 'li-extractor-v41']; " +
    "    for(var i=0; i<ids.length; i++) { var el = document.getElementById(ids[i]); if(el) el.remove(); } " +
    "    " +
    "    /* Configurações v41 */ " +
    "    var STORAGE_KEY = 'li_extractor_batch_v41'; " +
    "    var TURBO_KEY = 'li_extractor_turbo_v41'; " +
    "    var LAST_SIG_KEY = 'li_extractor_sig_v41'; " +
    "    var SEPARATOR = '\\n\\n<<<< PAGE_SPLIT_V41 >>>>\\n\\n'; " +
    "    " +
    "    /* Funções Auxiliares */ " +
    ...
    "      var pageCount = stored ? (stored.match(/<<<< PAGE_SPLIT_V41 >>>>/g) || []).length + 1 : (stored ? 1 : 0); " +
    ...
    "      } " +
    "      content += '</div>'; " +
    "      modal.innerHTML = '<div style=\"background:'+headerColor+';color:white;padding:12px;font-weight:bold;display:flex;justify-content:space-between\"><span>⚡ Sonda Isa (v41)</span><button id=\"btn-close\" style=\"background:none;border:none;color:white;cursor:pointer\">×</button></div>' + content; " +
    ...
    "         if(isAutoMode) { var m = document.getElementById('li-extractor-v41'); if(m) m.remove(); showModal('🔍 VERIFICANDO PÁGINA...'); } " +
    ...
    "             if (attempts > 10 || currentData.signature !== lastSig) { " +
    "                 clearInterval(checkInterval); " +
    "                 if(isAutoMode) { var m = document.getElementById('li-extractor-v41'); if(m) m.remove(); showModal('🔄 ROLANDO PAGINA...'); } " +
    "                 scrollToBottomSlowly(function() { " +
    "                     var freshData = getPageContent(); " +
    "                     var s = getStored(); " +
    "                     if (s) s += SEPARATOR; else s = 'LINKEDIN_EXTRACTOR_BATCH_START_V41\\n\\n'; " +
    "                     s += freshData.text; s += '\\n\\n(PAGE_SIGNATURE: ' + freshData.signature + ')'; " +
    "                     setStored(s); " +
    "                     setLastSig(freshData.signature); " +
    "                     if (isAutoMode) { " +
    "                         var m = document.getElementById('li-extractor-v41'); if(m) m.remove(); " +
    "                         showModal('➡️ INDO PARA PRÓXIMA PÁGINA...'); " +
    "                         if(clickNextPage()) { setTimeout(function() { runExtraction(null, true); }, 3000); } " +
    "                         else { setTurbo('false'); alert('🏁 FIM!'); var m = document.getElementById('li-extractor-v41'); if(m) m.remove(); showModal(null); } " +
    "                     } else { " +
    "                         var m = document.getElementById('li-extractor-v41'); if(m) m.remove(); showModal(null); " +
    "                     } " +
    "                 }); " +
    "             } else { " +
    "                 attempts++; " +
    "                 console.log('Aguardando mudança de conteúdo...'); " +
    "             } " +
    "         }, 1000); " +
    "    } " +
    "    if (isTurbo()) { runExtraction(null, true); } else { showModal(null); } " +
    "  } catch(e) { alert('Erro v41: ' + e.message); } " +
    "})();";
  ...
  return (
    <div className="bg-white rounded-lg border-2 border-indigo-100 p-6 mb-8 shadow-sm">
      <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
        ...
        Captura de Dados (Sonda Isa v41)
      </h3>
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
        <p className="text-sm text-blue-800">
          <strong>SMART AUTO-PILOTO (v41):</strong><br/>
          Clique em <strong>"🚀 ATIVAR AUTO-PILOTO"</strong>. <br/>
          O script agora <strong>espera a página mudar</strong> de verdade antes de extrair, garantindo que não pule páginas e salve todos os contatos.
        </p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
           ...
           <div className="p-6 bg-gray-100 rounded text-center border border-dashed border-gray-400 mt-4">
              <a ref={linkRef} onClick={(e) => e.preventDefault()} className="inline-block px-6 py-3 bg-indigo-700 text-white text-sm font-bold rounded shadow-md hover:bg-indigo-800 cursor-grab active:cursor-grabbing" title="Sonda Isa v41">
                Sonda Isa (v41)
              </a>
           </div>
        </div>
